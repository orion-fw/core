...
import * as playerConfig from "../config/player.json";
import {
  CharacterDataObject,
  JSONValue,
  PlayerDBObject,
...
import { UpdateCharacterDataClient, UpdatePlayerDataClient } from "./functions";
...
export class Player {
  ...
  constructor(
    ...
    if (!newPlayer && data) {
      ...
      this.setGroups(playerConfig.defaultGroup);
  ...
  public setGroups = (
    ...
    const groupObj: JSONValue = playerConfig.groups;
  ...
  public addGroups = (
    ...
    if (typeof groups === "string") {
      if (!groupObj[groups])
        return console.error(`Group ${groups} does not exist!`);
  ...
  public removeGroups = (
    ...
    if (typeof groups === "string") {
      ...
      if (typeof this._groups === "string")
        this._groups = playerConfig.defaultGroup;
      else
        this._groups.filter((group: string) => {
          if (group !== groups) return group;

          ExecuteCommand(
            `remove_principal player.${this._source} nw.${groups}`
          );
        });
    } else {
      if (typeof this._groups === "string") {
        ExecuteCommand(
          `remove_principal player.${this._source} nw.${this._groups}`
        );
        ...
        groups.forEach((group: string) => {
          ...
          if (Array.isArray(this._groups))
            this._groups.filter((_group: string) => {
              if (_group !== group) return _group;

              ExecuteCommand(
                `remove_principal player.${this._source} nw.${group}`
              );
            });
        });
      }
  ...
  public setCharacter = (
    ...
    const previousVal: Character | undefined = this._characters.get(
  ...
  public getCharacter = (citizenId: string): Character | undefined =>
    this._characters.get(citizenId);
  ...
  public switchCharacter = (
    ...
    if (updateClientData) {
      UpdatePlayerDataClient(
        ...
        "switch",
        "character",
        newChar.toClientObject(),
        previousVal
      );
      UpdateCharacterDataClient(
        this._source,
        this._currentChar.getCitizenId(),
        "switch",
        "character",
        newChar.toClientObject(),
        previousVal
  ...
  private _extraTables: { [key: string]: string } = {
  ...
  public deleteCharacter = (citizenId: string): void => {
    ...
    global.exports.oxmysql.query(
      "DELETE FROM characters WHERE citizenId = ?",
      [citizenId],
      () => {
    ...
    if (Object.keys(this._extraTables).length > 0) {
      for (const table in this._extraTables) {
        global.exports.oxmysql.query(
          `DELETE FROM ${table} WHERE ${this._extraTables[table]} = ?`,
          [citizenId]
        );
      }
    }
